...
**Standalone:** N/A — vanilla HTML, no `yarn start`
...
- **Deploy pipeline:** `deploy-to-fnb-pages.yml` → `scripts/deploy-fnb-static-pages.mjs` → `Linm.Web.FnB.Page.Index` với commit `fnb-demo@{version}`
...
# Mở trực tiếp hoặc serve tĩnh:
npx --yes serve . -p 9300
